    ...
    with st.chat_message("assistant"):
        ...
        if result:
            ...
            if intent in ("chitchat", "out_of_scope", "rejected"):
                if intent == "rejected":
                    msg = "⚠️ Your query was flagged for safety reasons. Please rephrase."
...
def _render_analysis(result: dict, ticker: str):
    ...
    with st.expander("📊 Recommendation Details", expanded=True):
        ...
        with col_a:
            ...
            for f in recommendation.get("supporting_factors", []):
                st.write(f"- {f}")
        with col_b:
            st.write("**⚠️ Dissenting Factors:**")
            for f in recommendation.get("dissenting_factors", []):
                st.write(f"- {f}")

    # ── Bull vs Bear Debate ──
    if debate_history:
        with st.expander("🐂 vs 🐻 Bull vs Bear Debate", expanded=True):
    ...
    with st.expander("📈 Technical Indicators"):
        ...
        if market_data.get("is_mock"):
            st.warning("⚠️ Using demo data — real-time data unavailable")
